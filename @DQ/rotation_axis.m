...
function ret = rotation_axis(dq)
    ...
    if(sin(phi) <= eps) %sin(pi) is not zero due to machine precision, hence if sin(phi) is smaller than eps (i.e., the spacing of floating point numbers) we consider it zero. 
