...
classdef DQ_SerialWholeBody < DQ_Kinematics
    ...
    methods
        function obj = DQ_SerialWholeBody(varargin)
            robot = varargin{1};
        ...
        function x = fkm(obj,q,ith)
            ...
            % link in the combined kinematic chain.
            %
            % FKM takes into account the reference frame.
            
            % In practical terms, this function is only a wrapper for the
            % FKM_BY_CHAIN();
            if nargin > 3
                error('Invalid number of arguments');
            elseif nargin == 3
                % find the FKM up to the ith link of the combined
                % kinematic chain
                [index,index_in_chain] = retrieve_chain_from_link(obj,ith);
                x = fkm_by_chain(obj,q,index, index_in_chain);
            else
                x = fkm_by_chain(obj,q);
            end
        end
        
        function x = fkm_by_chain(obj,q,ith, jth)
            % Returns the forward kinematic model of the whole-body chain.
            %
            % x = FKM_BY_CHAIN(q) receives the configuration vector q of the whole
            % kinematic chain and returns the pose of the last frame.
            % x = FKM_BY_CHAIN(q, ith) calculates the forward kinematics up to the ith
            % kinematic chain.
            % x = FKM_BY_CHAIN(q, ith, jth) calculates the forward kinematics up to
            % the jth link of the ith kinematic chain.
            %
            % FKM_BY_CHAIN takes into account the reference frame.
            if nargin > 4
                error('Invalid number of arguments');
            elseif nargin == 4 
        ...
        function plot(obj,q)
            ...
            for i = 2:length(obj.chain)
                ...
                if isa(obj.chain{1}, 'DQ_MobileBase')
                    current_base_frame = obj.fkm_by_chain(q,i-1);
                else
        ...
        function J = pose_jacobian(obj,q,ith)
            ...
            % This is a wrapper for the method POSE_JACOBIAN_BY_CHAIN.
            if nargin > 3
                error('Invalid number of parameters')
            elseif nargin == 3
                % find the Jacobian up to the ith link in the combined
                % kinematic chain
                [index,index_in_chain] = retrieve_chain_from_link(obj,ith);
                J = pose_jacobian_by_chain(obj,q,index,index_in_chain);
            else
                % find the jacobian of the whole chain
                J = pose_jacobian_by_chain(obj,q);
            end
        end
        
        function J = pose_jacobian_by_chain(obj,q,ith,jth)
            % Returns the whole-body pose Jacobian.
            %
            % J = POSE_JACOBIAN_BY_CHAIN(q) receives the configuration vector q of the whole
            % kinematic chain and returns the jacobian matrix J that satisfies
            % vec8(xdot) = J * q_dot, where q_dot is the configuration velocity
            % and xdot is the time derivative of the unit dual quaternion that
            % represents the end-effector pose.
            % J = POSE_JACOBIAN_BY_CHAIN(q, ith) calculates the Jacobian up to the ith
            % kinematic chain.
            % J = POSE_JACOBIAN_BY_CHAIN(q, ith,jth) calculates the Jacobian up to the 
            ...
            % Human-Robot Collaboration [Contribution à la manipulation à
            % deux bras : des manipulateurs à la collaboration
            % homme-robot]," Université Montpellier 2, 2011.
        ...
        function x = raw_fkm(obj,q,ith)
            % Analogous to FKM, but without considering base and
            % end-effector changes.
            %
            % x = RAW_FKM(q) receives the configuration vector q of the
            % whole kinematic chain and returns the pose of the last frame.
            % x = RAW_FKM(q, ith) calculates the forward kinematics up to
            % the ith link in the combined kinematic chain.
            %
            % RAW_FKM does not take into account the reference frame.
            
            % This is a wrapper for the method RAW_FKM_BY_CHAIN
            ...
            elseif nargin == 3
                ...
                x = raw_fkm_by_chain(obj,q, index, index_in_chain);
            else
                x = raw_fkm_by_chain(obj,q);
            end
        end
        
        function x = raw_fkm_by_chain(obj, q, ith, jth)
        % Analogous to FKM_BY_CHAIN, but without considering base and end-effector changes.
        %
        % x = RAW_FKM_BY_CHAIN(q) receives the configuration vector q of the whole
        % kinematic chain and returns the pose of the last frame.
        % x = RAW_FKM_BY_CHAIN(q, ith) calculates the forward kinematics up to the ith
        % kinematic chain.
        % x = RAW_FKM_BY_CHAIN(q, ith, jth) calculates the forward kinematics up to
        % the jth link of the ith kinematic chain.
        %
        % RAW_FKM_BY_CHAIN does not take into account the reference frame.
        ...
        function [index,index_in_chain] = retrieve_chain_from_link(obj,ith)
            ...
            nchain = 0;
            for j = 1:length(obj.chain)
                index = j; % we use this variable to store the jth subchain
                if nchain + obj.chain{j}.get_dim_configuration_space() < ith
                    % the desired link is in subsequent chains
                    nchain = nchain + obj.chain{j}.get_dim_configuration_space();
                else
                    % This is the subchain where the ith link is. Now we
                    % also calculate the index in that chain
                    index_in_chain = ith - nchain;
                    break;
                end
            end
        end
